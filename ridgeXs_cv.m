function [mse, rre_cvp, r0e_cvp, expval] = ridgeXs_cv(KFolds, timeVec, stim_is, observed, ...
...
for fold = 1:KFolds
    if verbose
        disp(['ridgeXs_cv: ' num2str(fold) '/' num2str(KFolds) 'folds']);
    end
    
    %training
    [rre_cvp(:,:,fold), r0e_cvp(fold)] = ...
        ridgeXs(timeVec(cvp.training{fold}), stim_is(:,cvp.training{fold}),...
