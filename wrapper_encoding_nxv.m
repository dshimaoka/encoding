...
if ~ispc
    ...
    %     }^H
    %     >> {^HUnable to resolve the name dataPaths.encodingSavePrefix.
    %         }^H
    %     >> >> {^HUnable to resolve the name dataPaths.stimSaveName.
    %         }^H
    %     >> >> >> >> >> >> {^HUnrecognized function or variable 'dsRate'.
    %     }^H
    %     >> >> >> >> >> >> {^HUnable to resolve the name dataPaths.imageSaveName.
    %         }^H
    %     >> {^HUnable to resolve the name stimInfo.height.
    %         }^H
    %     >> >> >> >> >> Loading tabular text datastore
    %     >> {^HUnable to resolve the name dataPaths.timeTableSaveName.
    %         }^H
    %     >> >> {^HUnable to resolve the name ds.VariableNames.
    %         }^H
    %     >> >> {^HUnable to resolve the name dataPaths.encodingSavePrefix.
