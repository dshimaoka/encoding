...
for iidx = 1:numel(IDs)
    ...
    set(gca,'clim',spdlim); set(gca,'ColorScale','log');
    set(gca,'xtick',[],'ytick',[]);
    for iarea = 1:numel(label)
        contour(areaMatrix{iarea},[.5 .5], 'linecolor',lcolor(iarea,:),'linewidth',3);
    end
    ...
    set(ax(1),'ColorOrderIndex',1);ylabel('sigma(deg)');xlabel('eccentricity(deg)');title('pRF size (deg)');
    set(ax(2),'ColorOrderIndex',1, 'ylim',[0 2.5]);ylabel('SF(cpd)');xlabel('eccentricity(deg)');title('SF (cpd)');
    set(ax(3),'ColorOrderIndex',1);ylabel('TF(Hz)');xlabel('eccentricity(deg)');title('TF (Hz)');
    set(ax(4),'ColorOrderIndex',1);ylabel('speed(deg/s)');xlabel('eccentricity(deg)');title('speed (deg/s)');
    set(ax(4),'ylim',spdlim,'yscale','log');
    
    %% binned eccentricity
    iq = [];
    for iarea = 1:numel(label)
        ax(5)=subplot(3,5,12);
        errorbar(mebins, RF_sigma_bin(:,iarea),ste_RF_sigma_bin(:,iarea),'color',lcolor(iarea,:));
        hold on;
        %plot(mebins, coef_RF_sigma_bin(1,iarea)+coef_RF_sigma_bin(2,iarea)*mebins, 'linewidth',2,'color',lcolor(iarea,:));
        
        ax(6)=subplot(3,5,13);
        iq(iarea)=errorbar(mebins, bestSF_bin(:,iarea),ste_bestSF_bin(:,iarea),'color',lcolor(iarea,:));
        hold on
        %plot(mebins, coef_bestSF_bin(1,iarea)+coef_bestSF_bin(2,iarea)*mebins, 'linewidth',2,'color',lcolor(iarea,:));
        
        ax(7)=subplot(3,5,14);
        iq(iarea)=errorbar(mebins, bestTF_bin(:,iarea),ste_bestTF_bin(:,iarea),'color',lcolor(iarea,:));
        hold on
        %plot(mebins, coef_bestTF_bin(1,iarea)+coef_bestTF_bin(2,iarea)*mebins, 'linewidth',2,'color',lcolor(iarea,:));
        
        ax(8)=subplot(3,5,15);
        iq(iarea)=errorbar(mebins, bestSPD_bin(:,iarea),ste_bestSPD_bin(:,iarea),'color',lcolor(iarea,:));
        hold on
    ...
    set(ax(8),'ColorOrderIndex',1, 'tickdir','out','ylim',spdlim,'yscale','log');
    %legend(iq, label,'location','northwest');
    linkaxes(ax,'x');set(ax,'xlim',eccLim);%%linkaxes([ax(1) ax(3)],'y');
    ...
    save(saveName, 'areaStats',...
        ...
        'coef_RF_sigma','coef_RF_sigma_bin','ebins','mebins',...
